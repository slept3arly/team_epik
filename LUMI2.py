...
def login():
    ...
    login_submitted = st.button("Login")
...
def signup():
    ...
    signup_submitted = st.button("Sign Up")

    if signup_submitted:
        # TO DO: implement signup logic
        st.session_state.logged_in = True
        st.write("Signed up successfully!")
...
def main():
    ...
    # Create tabs
    tab1, tab2, tab3 = st.tabs(["Home", "Habit Tracking", "Daily Routine Analysis"])
...
if __name__ == "__main__":
    ...
    if not st.session_state.logged_in:
        login_or_signup = st.selectbox("Do you want to login or sign up?", ["Login", "Sign Up"])
